...
/* Table sizes. */
#define GDT_SIZE (FIRST_LDT_INDEX + NR_TASKS + NR_PROCS)  //n ub gdt[]
					/* spec. and LDT's */
#define IDT_SIZE (IRQ8_VECTOR + 8)	/* only up to the highest vector */		//n ub idt[]
...
#define DS_INDEX             3	/* kernel DS */		//n ub prot_init()
#define ES_INDEX             4	/* kernel ES (386: flag 4 Gb at startup) */		//n ub prot_init()
#define SS_INDEX             5	/* kernel SS (386: monitor SS at startup) */
#define CS_INDEX             6	/* kernel CS */		//n ub prot_init()
#define MON_CS_INDEX         7	/* temp for BIOS (386: monitor CS at startup) */
#define TSS_INDEX            8	/* kernel TSS */		//n ub prot_init()
#define DS_286_INDEX         9	/* scratch 16-bit source segment */			//n ub prot_init()
#define ES_286_INDEX        10	/* scratch 16-bit destination segment */	//n ub prot_init()
...
#define FIRST_LDT_INDEX     15	/* rest of descriptors are LDT's */		//n ub prot_init()

#define GDT_SELECTOR      0x08	/* (GDT_INDEX * DESC_SIZE) bad for asld */
#define IDT_SELECTOR      0x10	/* (IDT_INDEX * DESC_SIZE) */
#define DS_SELECTOR       0x18	/* (DS_INDEX * DESC_SIZE) */		//n ub prot_init()
#define ES_SELECTOR       0x20	/* (ES_INDEX * DESC_SIZE) */
#define FLAT_DS_SELECTOR  0x21	/* less privileged ES */		//n ub phys2seg
...
#define CS_LDT_INDEX         0	/* process CS */		//n ub alloc_segments
#define DS_LDT_INDEX         1	/* process DS=ES=FS=GS=SS */	//n ub alloc_segments
#define EXTRA_LDT_INDEX      2	/* first of the extra LDT entries */

/* Privileges. */		//n ub  prot_init
...
/* Exception vector numbers. */  //n ub prot_init()
...
/* Selector bits. */
#define TI                0x04	/* table indicator */  //n ub alloc_segments
...
#define DESC_SIZE            8	/* sizeof (struct segdesc_s) */		//n ub prot_init()  //n ub alloc_segments

/* Base and limit sizes and shifts. */
#define BASE_MIDDLE_SHIFT   16	/* shift for base --> base_middle */  //n ub sdesc

/* Access-byte and type-byte bits. */
#define PRESENT           0x80	/* set for descriptor present */	//n ub prot_init() init_codeseg
#define DPL               0x60	/* descriptor privilege level mask */	
#define DPL_SHIFT            5	//n ub prot_init()
#define SEGMENT           0x10	/* set for segment-type descriptors */		//n ub init_codeseg

/* Access-byte bits. */
#define EXECUTABLE        0x08	/* set for executable segment */		//n ub init_codeseg
...
#define READABLE          0x02	/* set for readable segment if executable */	//n ub init_codeseg
...
#define AVL_286_TSS          1	/* available 286 TSS */		//n ub TSS_TYPE
#define LDT                  2	/* local descriptor table */	//n ub prot_init()
...
#define INT_286_GATE         6	/* interrupt gate, used for all vectors */		//n ub INT_GATE_TYPE
...
/* Base and limit sizes and shifts. */
#define BASE_HIGH_SHIFT     24	/* shift for base --> base_high */		//n ub sdesc
#define BYTE_GRAN_MAX   0xFFFFFL   /* maximum size for byte granular segment */   //n ub sdesc
#define GRANULARITY_SHIFT   16	/* shift for limit --> granularity */		//n ub sdesc
#define OFFSET_HIGH_SHIFT   16	/* shift for (gate) offset --> offset_high */
#define PAGE_GRAN_SHIFT     12	/* extra shift for page granular limits */		//n ub sdesc

/* Type-byte bits. */
#define DESC_386_BIT  0x08 /* 386 types are obtained by ORing with this */		//n ub TSS_TYPE    INT_GATE_TYPE
				/* LDT's and TASK_GATE's don't need it */

/* Granularity byte. */
#define GRANULAR  	  0x80	/* set for 4K granularilty */			//n ub sdesc
#define DEFAULT   	  0x40	/* set for 32-bit defaults (executable seg) */		//n ub sdesc
