 ...
 *===========================================================================*/
 ...
 //n  normal execution of the system ,called by mpx.s  
 //sec :declaration,init interrupt controller, setup process and privilege table...
 //In many C programs main is a loop, but in the MINIX 3 kernel its job is done once the initialization is complete. The call to restart(in mpx386.s) in the end starts the first queued process. Control never returns to main.
...
/* Start the ball rolling. */
  ...
  /*
  The parameter (1) in the call tells intr_init that it is initializing for MINIX 3. With a parameter (0) it can be called to reinitialize the hardware to the original state when MINIX 3 terminates and returns control to the boot monitor. 
  */

  //n  Because intr_init is very dependent upon the hardware, the procedure is in a separate file 
  //n  Intr_init ensures that any interrupts that occur before initialization is complete have no effect. 
  intr_init(1);

//n  init pproc_addr[],ppriv_addr[] . what about the process table and the privilege table(proc[] priv[])?    

  /* Clear the process table. Anounce each slot as empty and set up mappings 
   * for proc_addr() and proc_nr() macros. Do the same for the table with 
   * privilege structures for the system processes. 
   */
  for (rp = BEG_PROC_ADDR, i = -NR_TASKS; rp < END_PROC_ADDR; ++rp, ++i) {//n the 4 tasks are assigned negative number to distinguish from other processes
  ...
  for (i=0; i < NR_BOOT_PROCS; ++i) {
	  //n get info from boot image array(image[i]) and assign to current proc(rp)
 ...
 *				prepare_shutdown			     *
 *===========================================================================*/
 //n signals all system processes with a SIGKSTOP signal (system processes cannot be signaled in the same way as user processes). Then it sets a timer to allow all the system process time to clean up before it calls the final procedure here, shutdown
 ...
 *				shutdown 				     *
 *===========================================================================*/
 //n Shutdown will normally return control to the MINIX 3 boot monitor. To do so the interrupt controllers are restored to the BIOS settings by the intr_init(0) call
