...
//n used to define tss var below
struct tss_s {
  reg_t backlink;
 ...
 *				prot_init				     *
 *===========================================================================*/
//n used in cstart(), see the comment where this is used.
//n just init each slot of gdt, gate_table(near the end),tss(near & at the end) defined above
...
//n functions are defined in mpx386.s
//n for vectors  of the first part , definitions are in several parts ,but the number continuse from 0to 16
  ...
  * (u32_t *) dtp->base = vir2phys(idt);
   ...
   */
   //n init ldt  slots,total of proc number,each for one process in proc[] table.
  ...
  for (gtp = &gate_table[0];  gtp < &gate_table[sizeof gate_table / sizeof gate_table[0]]; ++gtp) {
	int_gate(gtp->vec_nr, (vir_bytes) gtp->gate, PRESENT | INT_GATE_TYPE | (gtp->privilege << DPL_SHIFT));
  }
 ...
 *				sdesc					     *
 *===========================================================================*/
 //n IA32 manual vol3 3.4.5 Segment Descriptors
 //n used in init_codeseg,init_dataseg
 ...
 */
  phys_bytes base;
  struct segdesc_s *segdp;//n protected mode segment descriptor

  if (! machine.protected) {
	//real mode,
	base = hclick_to_physb(seg);//base=seg *16  
  } else {
	//n protected mode
	//n  see intel IA32 manual v3 3.4.2,3.5.1
	segdp = &gdt[seg >> 3];
	//n concanate base address
