 ...
 *				cstart					     *
 *===========================================================================*/
//n called by mpx386.s during init code execution.
//n just set fields value of kinfo & machine structure,set mon_return, and call prot_init()
 ...
 */
  char params[128*sizeof(char *)];		/* boot monitor parameters */  //n for setting machine fields
  register char *value;				/* value in key=value pair */ //n temporary var
...
//n kinfo should be the one in kernel/glo.h, kernel/glo.h is included by kernel.h
//n copies the boot parameters to the kernel's memory, and it scans them, using the function get_value to search for parameter names and return corresponding value strings. 
//n All this information is stored in global variables, for access when needed by any part of the kernel code.

  /* Record where the kernel and the monitor are. */
  //n etext end are defined in lib/end/.?
  ...
  //n see mpx386.s 'lgdt	(_gdt+GDT_SELECTOR)','lidt...'  
  prot_init();
  ...
  phys_copy(kinfo.params_base, vir2phys(params), kinfo.params_size);//n kernel/klib386.s _phys_copy
