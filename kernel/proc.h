...
/* M68000 specific registers and FPU details go here. */
  ...
  proc_nr_t p_nr;		/* number of this process (for fast access) */  //n ub mini_send
  struct priv *p_priv;		/* system privileges structure */
  char p_rts_flags;		/* SENDING, RECEIVING, etc. */ //n rts=request to send(from searching rts in google)? ub  mini_send

  char p_priority;		/* current scheduling priority */
  char p_max_priority;		/* maximum scheduling priority */
  
  //n p_ticks_left,p_quantum_size are inited in main(). p_ticks_left is inited to be the same as p_quantum_size
  ...
  //n   All processes wanting to send to this are strung together on a linked list, with the p_caller_q field pointing to the process at the head of the queue. see   Fig. 2-42
   //n The example of Fig. 2-42(a) shows what happens when process 3 is unable to send to process 0. If process 4 is subsequently also unable to send to process 0, we get the situation of Fig. 2-42(b).
  struct proc *p_caller_q;	/* head of list of procs wishing to send */		//n ub & set at the end of mini_send
  struct proc *p_q_link;	/* link to next proc wishing to send */			//n ub & set at the end of mini_send
  message *p_messbuf;		/* pointer to passed message buffer */			//n ub mini_send
  proc_nr_t p_getfrom;		/* from whom does process want to receive? */   //n ub mini_send
  proc_nr_t p_sendto;		/* to whom does process want to send? */		//n ub mini_send
 ...
 */
#define NR_SCHED_QUEUES   16	/* MUST equal minimum priority + 1 */   //n ub  pick_proc()
