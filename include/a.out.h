...
#ifndef _AOUT_H
#define _AOUT_H
//n a.out: The name stands for assembler output 
/*n
a.out format was superseded by the COFF format in AT&T Unix System V, which was in turn superseded by the ELF format in System V Release 4.

Linux also used a.out until kernel 1.2 , when it was superseded by ELF for that platform as well.  Linux's transition to ELF was more or less forced due to the complex nature of building a.out shared libraries on that platform , which included the need to register the virtual address space at which the library was located with a central authority, as the a.out ld.so in Linux was unable to relocate shared libraries.
...
An a.out file consists of up to seven sections
exec header
text segment
data segment...
...
*/
//n from wiki a.out
//n http://man.cat-v.org/unix-6th/5/a.out  a.out manual page for Version 6 Unix 
struct	exec {			/* a.out header */
  unsigned char	a_magic[2];	/* magic number */
  unsigned char	a_flags;	/* flags, see below */   //n ub main()
  unsigned char	a_cpu;		/* cpu id */
  unsigned char	a_hdrlen;	/* length of header */
  unsigned char	a_unused;	/* reserved for future use */
  unsigned short a_version;	/* version stamp (not used at present) */
  long		a_text;		/* size of text segement in bytes */  //n ub main()
  long		a_data;		/* size of data segment in bytes */   //n ub main()
  long		a_bss;		/* size of bss segment in bytes */
  long		a_entry;	/* entry point */
  long		a_total;	/* total memory allocated */ //n ub main()
  long		a_syms;		/* size of symbol table */ //n ub main()
