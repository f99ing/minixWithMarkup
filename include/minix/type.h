...
//n one such struct is inited in cstart()
struct kinfo {
...
//n used to define machine var, one such struct is inited in cstart()
struct machine {
  int pc_at;
  int ps_mca;	//n ub  clock_handler
  int processor;
  int protected;//n if in real or protected mode.This of one machine struct is set at the beginning of cstart()(=1)
  int vdu_ega;
  int vdu_vga;
};
